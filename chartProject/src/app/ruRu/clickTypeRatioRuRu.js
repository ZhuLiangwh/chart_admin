define([
    ...
    var diName = 'ClickTypeRatioRuRuListCtrl';
    return {
        __register__: function(mod) {
            mod.controller(diName, ['$scope', 'logger', 'ds.clickTypeRatioRuRu', ClickTypeRatioRuRuListCtrl]);
            return mod;
        }
    };

    function ClickTypeRatioRuRuListCtrl($scope, logger, DS) {
        var apiParams = {} /*,chartTypes =["pie"]*/ ;


        $scope['isCollapse0'] = false;
        $scope['config0'] = {
            "title": {
                "text": "Version Distribution(Pie Chart)"
            },
            "type": "pie",
            "series": [{
                "index": 1,
